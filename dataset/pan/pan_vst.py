...
tt_root_dir = './data/'
tt_train_data_dir = tt_root_dir + 'VinText/images/'
tt_train_gt_dir = tt_root_dir + 'VinText/groundtruths/'
tt_test_data_dir = tt_root_dir + 'VietSceneText_test/images/'
tt_test_gt_dir = tt_root_dir + 'VietSceneText_test/groundtruths_word/'
...
def get_ann(img, gt_path):
	...
	for i, lines in enumerate(data):
		X = np.array([lines[:-1][::2]])
		Y = np.array([lines[:-1][1::2]])
...
class PAN_VST(data.Dataset):
	def __init__(self,
		...
		for data_dir, gt_dir in zip(data_dirs, gt_dirs):
			...
			img_names.extend([img_name for img_name in mmcv.utils.scandir(data_dir, '.bmp')])
...
if __name__ == "__main__":
	...
	data = data_loader.prepare_train_data(2)
